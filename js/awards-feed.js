(function () {
  ...
  function card(a) {
    ...
    var ticker = a.ticker ? '<div class="text-xs mt-2"><span class="bronze-text">Ticker:</span> ' + esc(a.ticker) + "</div>" : '<div class="text-xs mt-2 text-[#8A8F82]">No verified public ticker</div>';
    var title = a.title ? '<div class="text-sm bronze-text mb-1">' + esc(a.title) + "</div>" : "";
    var status = a.status ? '<span class="badge-status badge-' + esc(a.status) + '">' + esc(a.status) + "</span>" : "";
    ...
    el.innerHTML =
      ...
      (a.source_url ? '<a href="' + esc(a.source_url) + '" target="_blank" rel="noopener" class="inline-block mt-3 text-xs bronze-link">Official source →</a>' : "");
    return el;
  }
  var DATA = null;
  ...
  function apply(sector) {
    ...
    root.innerHTML = "";
    if (!shown.length) {
      root.innerHTML = '<p class="text-sm text-[#8A8F82] col-span-full">No awards in this filter.</p>';
    } else {
      shown.forEach(function (a) { root.appendChild(card(a)); });
    }
    var meta = document.getElementById("awards-meta");
    if (meta && DATA) {
      meta.textContent = shown.length + " items · snapshot " + (DATA.generated_at || "") + " · window " + ((DATA.window && DATA.window.start) || "") + " to " + ((DATA.window && DATA.window.end) || "") + ". Status: FINAL / CONDITIONAL / LOI / UCA. Not investment advice.";
    }
  ...
  document.querySelectorAll("[data-filter-sector]").forEach(function (btn) {
    btn.addEventListener("click", function () {
      ...
      if (history.replaceState) {
        ...
        if (s === "all") url.searchParams.delete("sector"); else url.searchParams.set("sector", s);
  ...
  fetch("data/awards.json")
    ...
    .then(function (data) {
      ...
      if (typeof data.awards === "string") {
        root.innerHTML = '<p class="text-sm text-[#8A8F82]">awards.json is a stub (awards is not an array). Reload after the snapshot is restored.</p>';
        return;
      }
      apply(current);
    })
    .catch(function () {
      root.innerHTML = '<p class="text-sm text-[#8A8F82] col-span-full">Could not load data/awards.json. Serve over HTTP. Empty state — no invented awards.</p>';
      var meta = document.getElementById("awards-meta");
