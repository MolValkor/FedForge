(function () {
  ...
  function card(a) {
    var ticker = a.ticker
      ? '<div class="text-xs"><span class="bronze-text">Ticker:</span> ' + esc(a.ticker) + "</div>"
      : "";
    ...
    el.innerHTML =
      '<div class="flex justify-between items-start mb-4 gap-3">' +
        '<div>' +
          '<div class="text-xs text-[#8A8F82]">' + esc(sectorLabel(a.sector)) + "</div>" +
          '<div class="text-xl font-semibold">' + esc(a.recipient_name || "") + "</div>" +
        "</div>" +
        '<span class="px-2 py-1 badge-green text-xs rounded whitespace-nowrap">' + esc(fmtDate(a.date)) + "</span>" +
      "</div>" +
      '<div class="text-lg font-semibold amount-pos mb-2">' + esc(fmtMoney(a.amount)) + "</div>" +
      '<p class="text-sm text-[#8A8F82] mb-4">' + esc(a.description || "") + "</p>" +
      ticker +
      (a.source_url
        ? '<a href="' + esc(a.source_url) + '" target="_blank" rel="noopener" class="inline-block mt-3 text-xs bronze-link">USAspending source →</a>'
        : "");
    return el;
  }

  function render(data) {
    var awards = (data && Array.isArray(data.awards)) ? data.awards : [];
    root.innerHTML = "";
    if (!awards.length) {
  ...
  fetch("data/awards.json")
    ...
    .then(function (data) {
      if (data && Array.isArray(data.awards) && data.awards.length) render(data);
    })
    .catch(function () {});
})();
